...
class Simulator(gym.Env):
    ...
    def reset(self):
        ...
        if self.user_tile_start:
            logger.info('using user tile start: %s' % str(self.user_tile_start))
        ...
        x = (3 + 0.5) * self.road_tile_size
        z = (1 + 0.5) * self.road_tile_size
        propose_angle = 0
        propose_pos = np.array([x, 0, z])
    ...
    def get_agent_info(self):
        ...
        if self.full_transparency:
            ...
            # (0,0 is bottom left)
            #p = self.cartesian_from_weird(pos, angle)
            #info['hz'] = p
